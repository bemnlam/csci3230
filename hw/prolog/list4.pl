...
update_board([L1,L2,L3|L4],Play,[AnsRow|AnsBoard]):-
	...
	update_board([AnsRow,L3|L4],Play,AnsBoard).

update_board([],[L1,L2|L3],Play,[AnsRow|AnsBoard]):-
	update_row_notop([],L1,L2,Play,AnsRow),
	update_board([AnsRow,L2|L3],Play,AnsBoard).
...
life_and_death2(Raw,move(Y,X),LDBoard):- 
...
life_and_death(Raw,move(Y,X),LDBoard):-get_board_ele(Raw,Y,X,Play), init_board(Raw,Board,Num), check_board(Board,Play,LDBoard).